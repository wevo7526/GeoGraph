...
def find_analogues(
    ...
    *,
    ...
    try:
        rows = kuzu_store.query(conn, _EVENT_QUERY)
        ...
        candidates = []
        for row in rows:
            if row["node_id"] == query_ref:
                continue
            if not regimes.comparable(
                query_date, str(row["event_time"]), kind=regime_kind
            ):
                continue
            candidates.append((_similarity(query, row), row))
